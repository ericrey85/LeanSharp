...
namespace LeanSharp.Tests
{
    public class RopExtensionsTests
    {
        ...
        [Fact]
        public void TryCatch_AppliesMappingFunctionIfNoExceptionsAreThrown()
        {
            var result = Result<int, string>.Succeeded(4);

            var newResult = result.Try(r => Result<int, Exception>.Succeeded(r + 6));

            Assert.Equal(10, newResult.Success);
        }

        [Fact]
        public void TryCatch1_AppliesMappingFunctionIfNoExceptionsAreThrown()
        {
            var result = Result<int, string>.Succeeded(4);

            var newResult = result.Try(r => r + 6);

            Assert.Equal(10, newResult.Success);
        }

        [Fact]
        public void TryCatch_ReturnsExceptionAsFailedResultIfExceptionIsThrown()
        {
            var result = Result<int, string>.Succeeded(4);

            var newResult = result.Try(r =>
            {
                throw new Exception("Test exception");
                return Result<int, Exception>.Succeeded(r + 6);
            });

            Assert.True(newResult.IsFailure);
            Assert.Equal("Test exception", newResult.Failure.Message);
        }

        [Fact]
        public async Task TryCatchAsync_AppliesMappingFunctionIfNoExceptionsAreThrown()
        {
            var result = Result<int, string>.Succeeded(4);

            var newResult = await result.TryAsync(async r => await Task.FromResult(Result<int, Exception>.Succeeded(r + 6)));

            Assert.Equal(10, newResult.Success);
        }

        [Fact]
        public async Task TryCatchAsync_ReturnsExceptionAsFailedResultIfExceptionIsThrown()
        {
            var result = Result<int, string>.Succeeded(4);

            var newResult = await result.TryAsync(async r =>
            {
                throw new Exception("Test exception");
                return await Task.FromResult(Result<int, Exception>.Succeeded(r + 6));
            });
