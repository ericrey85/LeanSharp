...
namespace LeanSharp.Extensions
{
    public static class TraversableExtensions
    {
        ...
        public static AsyncLazyPipeline<Result<TSuccess, TFailure>> ToPipeline<TSuccess, TFailure>(
        ...
        public static AsyncLazyPipeline<Result<TSuccess, TFailure>> FlipMerge<TSuccess, TFailure>(
            this Result<AsyncLazyPipeline<Result<TSuccess, TFailure>>, TFailure> result)
        => result.EitherFold(r => r.Success, e => CreatePipeLine.Return(Result<TSuccess, TFailure>.Failed(e.Failure)));

        public static AsyncLazyPipeline<Result<TSuccess, TFailure>> FlipMerge<TSuccess, TFailure>(
            this AsyncLazyPipeline<Result<AsyncLazyPipeline<TSuccess>, TFailure>> pipeLine)
        => pipeLine
            .SelectMany(result =>
                result.EitherFold(
